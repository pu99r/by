...
bot.on("message", async (msg) => {
    ...
    for (const u of users) {
      const caption =
        u.status === "mes"
          ...
          : "🚀 Активируйте подписку всего за 0.19Br и начните получать прибыль уже сегодня!";
  ...
  if (text.startsWith("/start")) {
    ...
    if (rawParam?.endsWith("_al2")) {
      generatedLink = `https://justonesec.ru/stream/cpamode249aa?cid=${baseCid}${sub1}`;
  ...
  if (text === "Как получить") {
    ...
    await safeSendPhoto(chatId, path.join(__dirname, "photo", "2.webp"), {
      caption:
        ...
        "2. ⚠️ ВАЖНО! Оплатите 0.19Br и подключите PRO доступ к аналитике на 5 дней;\n" +
  ...
  if (data.startsWith("q2_") && userStates[chatId]?.state === STATES.Q2) {
    ...
    const q = await safeSendMessage(chatId, "Сколько хотите зарабатывать?", {
      reply_markup: {
        inline_keyboard: [
          [
            { text: "4000Br", callback_data: "q3_50" },
            { text: "6000Br", callback_data: "q3_70" },
          ],
          [
            { text: "8000Br", callback_data: "q3_120" },
            { text: "10000Br", callback_data: "q3_300" },
          ],
        ],
      },
    });
...
async function sendFinal(chatId) {
  ...
  await safeSendMessage(
    chatId,
    "🎉 Стратегия готова!\n\n💰 Потенциальный доход: 8000Br/день",
    {
      reply_markup: {
